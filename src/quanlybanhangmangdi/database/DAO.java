...
public class DAO {
	...
	public static ArrayList<NhanVienDTO> getDuLieuNhanVien() {
		...
		try {
			while (rs.next()) {
				...
				danhSachNhanVien.add(new NhanVienDTO(maNhanVien, maChucVu, hoTen, gioiTinh, ngaySinh, dienThoai, diaChi,
						taiKhoan, matKhau));
			}
	...
	public static ArrayList<DonHangDTO> getDuLieuDonHang() {
		...
		try {
			while (rs.next()) {
				...
				danhSachDonHang.add(new DonHangDTO(maDon, maNV, d, maApp, maDonApp, tongGia, chietKhau, phiDichVu,
						tongTienThu, getCacChiTietDon(maDon)));
			}
	...
	public static ArrayList<DonHangTable> getDuLieuDonHangAnTable() {
		...
		try {
			while (rs.next()) {
				...
				int phiDichVu = rs.getInt("PhiDichVu");
				int tongTienThu = rs.getInt("TongTienThu");
				danhSachDonHang.add(new DonHangTable(maDon, Test.nhanVien.getHoTen(), sdf.format(d),
						getTenAppTuMaApp(maApp), maDonApp, tongGia, chietKhau, phiDichVu, tongTienThu));
			}
			return danhSachDonHang;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public static ArrayList<TablePhieuChi> getCacPhieuChiTheoNgay(LocalDate ld, int trangthai) {
		...
		try {
			while (rs.next()) {
				...
				String query1 = "SELECT * FROM NhanVien" + " WHERE ma = " + "\"" + manhanvien + "\"";
	...
	public static ArrayList<ChiTietChiTable> getChiTietChi(String maphieuchi) {
		...
		String sql = "SELECT * FROM ChiTietChi" + " WHERE maphieuchi = " + "\"" + maphieuchi + "\"";
		ResultSet rs = DataHelper.execQuery(sql);
		try {

			while (rs.next()) {
				String manguyenlieuchitietchi = rs.getString("manl");
				String s = "SELECT * FROM NguyenLieu" + " WHERE ma = " + "\"" + manguyenlieuchitietchi + "\"";
	...
	public static boolean luuDatabase(PhieuChi pc) throws SQLException {
		...
		try {
			...
			String sql = "INSERT INTO PhieuChi(ma,manhanvien,ngay,tonggia) \r\n" + "VALUES (\"" + mapc + "\"," + "\""
					+ pc.getManhanvien() + "\"," + "\"" + sdf1.format(pc.getNgay()) + "\"," + "\"" + pc.getTonggia()
					+ "\")";
	...
	public static boolean suaNguyeLieu(NguyenLieu nl) {
		try {
			String sql = "UPDATE nguyenlieu SET ten = '" + upperCaseFirst(nl.getTen()) + "', gia =  " + nl.getGia()
					+ ", soluong = " + nl.getSoluong() + " " + "WHERE ma =" + nl.getMa();
	...
	public static boolean luuNguyenLieu(NguyenLieu nl) {
		...
		String sql = "INSERT INTO nguyenlieu(ma,ten,gia,soluong)" + " VALUE  ('" + manl + "', '" + tennl + "', "
				+ nl.getGia() + ", " + nl.getSoluong() + ")";
		DataHelper.execAction(sql);
		return true;
	}
}
